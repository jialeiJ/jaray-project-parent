  ...
  <div>
    <div id="table">
      <div style="margin-bottom: 10px">
        ...
        <el-button v-if="hasPermission('sys:user:add')" type="success" plain @click="addLeaveApply">增加</el-button>
    ...
    <!-- 流程图 -->
    <view-flow-diagram ref="viewFlowDiagram" :url="url" :task-list="taskList" />

    <!-- 新增 -->
    <add-leave-apply ref="addLeaveApply" :status-options="statusOptions"/>

    <!-- 详情 -->
    <view-leave-apply ref="viewLeaveApply" :status-options="statusOptions"/>

    <!-- 编辑 -->
    <edit-leave-apply ref="editLeaveApply" :status-options="statusOptions"/>
  </div>
</template>
...
import addLeaveApply from '@views/workflow/leave/apply/addLeaveApply'
import editLeaveApply from '@views/workflow/leave/apply/editLeaveApply'
import viewLeaveApply from '@views/workflow/leave/apply/viewLeaveApply'
import viewFlowDiagram from '@views/workflow/leave/apply/viewFlowDiagram'

export default {
  name: 'LeaveApply',
  components: { iTable, iPagination, addLeaveApply, editLeaveApply, viewLeaveApply, viewFlowDiagram },
  data() {
    return {
      // label宽度
      ...
      tableTitle: [
        ...
        { prop: 'operation', label: '操作', fixed: 'right', width: 225,
          operation: [
            { name: '流程图', style: 'primary', clickFun: this.viewFlowDiagram, disabled: this.hasPermission('sys:user:view') },
            { name: '查看', style: 'primary', clickFun: this.viewLeaveApply, disabled: this.hasPermission('sys:user:view') },
            { name: '修改', style: 'primary', clickFun: this.editViewLeaveApply, disabled: this.hasPermission('sys:user:view') }
          ]
        }
      ],
      tableData: [],
  ...
  methods: {
    ...
    editViewLeaveApply: function(row) {
      const that = this
      that.$refs['editLeaveApply'].openDialog(row)
    },
    viewFlowDiagram: function(row) {
      const that = this
      that.$refs['viewFlowDiagram'].openDialog(row)
      ...
      // 调用接口
      that.url = LEAVE_APPLY_API.viewFlowDiagram(params)

      // 调用接口
      LEAVE_APPLY_API.findHistory(params).then(function(result) {
        if (result.code === 200) {
          console.log(result.map.taskList)
          that.taskList = result.map.taskList
        }
      })
    },
    initTable: function() {
      ...
      // 调用接口
      LEAVE_APPLY_API.findLeaveList(params).then(function(result) {
        if (result.code === 200) {
