  ...
  <div id="edit">
    <el-dialog :visible.sync="editDialogFormVisible" title="编辑">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editLeaveApprove">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    editLeaveApprove: function() {
      const that = this
      // 定义请求参数
      const params = that.editForm
      // 调用接口
      LEAVE_APPLY_API.approveLeave(params).then(function(result) {
        if (result.code === 200) {
